...
def main(args):
    ...
    for f in files:
        ...
        r_a = Rebust_Attack(args.attack_method, image_path,
                            label_path, image_origin_path,
                            label_origin_path, int(args.GPU_Config[0]), args.GPU_Config[1], 10, args.Scale_ImageSize,
                            args.Crop_ImageSize, \
                            args.model, args.model_dir, args.defense_model, \
                            args.model_defence_dir, args.data_type, args.IS_WHITE, \
                            args.IS_SAVE, args.IS_COMPARE_MODEL, args.IS_TARGETTED, args.save_path, args.save_method, \
                            args.black_Result_dir, args.batch_size)
        ...
        r_e = Rebust_Evaluate(tensor_adv_xs, tensor_cln_xs, tensor_cln_ys, tensor_adv_ys, tensor_ys_targeted,
                              evaluation_method=args.evaluation_method, IS_PYTHORCH_WHITE=args.IS_PYTHORCH_WHITE, \
                              IS_COMPARE_MODEL=args.IS_COMPARE_MODEL, IS_TARGETTED=args.IS_TARGETTED)
        #获取模型信息
        ...
        if not args.IS_COMPARE_MODEL:
            ...
            rst = r_e.gen_evaluate(black_origin_outputs, black_adv_outputs, black_defense_origin_outputs,
                                   black_defense_adv_outputs)

        #log信息

        SaveWithJson_Result(args.save_visualization_base_path, "table_list", attName[0], f, args.evaluation_method,
                            rst)
        print("Evaluation output : ", rst)
        update_current_status(args.save_visualization_base_path, attName[0],
                              100 * int(f.replace(".xml", "").split("_")[-1]) / len(files))
        topk_show_list = [0]
        Save_Eval_Visualization_Result(attName,args.data_type,f,args.Dict_path,device,adv_xs_npy,args.save_visualization_base_path,\
                                   args.IS_COMPARE_MODEL,args.model,args.defense_model,\
                                   model, defense_model,args.CAM_layer,\
...
if __name__ == '__main__':
    ...
    parser.add_argument(
        ...
        type=str,
        nargs='*',
        default=["../Datasets/ImageNet/images/","../Datasets/ImageNet/val_1.txt","../Datasets/ImageNet/images/","../Datasets/ImageNet/val_1.txt"])
    ...
    parser.add_argument(
        '--Dict_path',
        type=str,
        default="./dict_lists/ImageNet_12_dict.txt")
    parser.add_argument(
        '--defense_model',
        type=str,
        default = 'torchvision.models.vgg19' )
    parser.add_argument(
        '--model',
        type=str,
    ...
    parser.add_argument(
        '--save_path',
        type=str,
        default='ImageNet_Attack_generation/')
    parser.add_argument(
        '--save_method',
        type=str,
        default='ImageNet')
