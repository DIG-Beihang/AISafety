...
def save_numpy(X  #全部的数据
    ...
    if not IsTargeted:
       ...
       np.save(path+'{}_{}_origin_labels.npy'.format(datasetType, number),
            np.array(ys))
       print("save the npy file in path :", path+'{}_{}_origin_labels.npy'.format(datasetType, number))
       np.save(path+'{}_{}_origin_inputs.npy'.format(datasetType,number ),
            np.array(X_shuffe/255))
       print("save the npy file in path :", path + '{}_{}_origin_inputs.npy'.format(datasetType, number))
    else:
       ...
       # print("A")
        ...
        np.save(path+'{}_{}_target_labels.npy'.format(datasetType, number),
                np.array(ys))
        print("save the npy file in path :", path + '{}_{}_target_labels.npy'.format(datasetType, number))


    print("save model is :",shuff,"\nIsTargeted :",IsTargeted,"\nsample class number is: ",
