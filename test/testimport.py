...
#####不同的攻击算法的配置参数，循环调用测评过程#####
    for f in files:
        ...
        r_a = Rebust_Attack(args.attack_method, image_path,
                            ...
                            label_origin_path, int(args.GPU_Config[0]), args.GPU_Config[1], 0, args.Scale_ImageSize,
                            args.Crop_ImageSize, \
        ...
        update_current_status(args.save_visualization_base_path, attName[0], 100 * int(f.replace(".xml", "").split("_")[-1]) / len(files))
