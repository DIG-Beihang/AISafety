...
PYTORCH_PRETRAINED_BERT_CACHE = nlp_cache_dir
...
def cached_path(url_or_filename, cache_dir=None):
    ...
    if parsed.scheme in ("http", "https", "s3"):
        ...
        return get_from_cache(url_or_filename, cache_dir)  # noqa: F821
...
class BertConfig(object):
    ...
    def __init__(
        ...
        if isinstance(vocab_size_or_config_json_file, str) or (
            ...
            and isinstance(vocab_size_or_config_json_file, unicode)  # noqa: F821
        ):
...
def _generate_relative_positions_embeddings(length, depth, max_relative_position=127):
    ...
    embeddings = torch.matmul(
        one_hot_relative_positions_matrix, embeddings_table_tensor
    )
...
class NeZhaSelfAttention(nn.Module):
    ...
    def forward(self, hidden_states, attention_mask):
        ...
        relations_keys = (
            self.relative_positions_embeddings.detach()
            .clone()[:to_seq_length, :to_seq_length, :]
            .to(device)
        )
...
class BertIntermediate(nn.Module):
    def __init__(self, config):
        ...
        if isinstance(config.hidden_act, str) or (
            sys.version_info[0] == 2
            and isinstance(config.hidden_act, unicode)  # noqa: F821
        ):
...
class BertForJointLSTM(BertPreTrainedModel):
    ...
    def forward(
        ...
        encoded_layers, attention_layers, pooled_output = self.bert(
        ...
        if intent_labels is not None and slot_labels is not None:
            ...
            if attention_mask is not None:
                ...
                active_slot_logits = slot_logits.view(-1, self.num_slot_labels)[
                    active_slot_loss
                ]
    ...
    for name, array in zip(names, arrays):
        ...
        for m_name in name:
            if re.fullmatch(r"[A-Za-z]+_\d+", m_name):
                ln = re.split(r"_(\d+)", m_name)
            else:
                ln = [m_name]
            if ln[0] == "kernel" or ln[0] == "gamma":
                pointer = getattr(pointer, "weight")
            elif ln[0] == "output_bias" or ln[0] == "beta":
                pointer = getattr(pointer, "bias")
            elif ln[0] == "output_weights":
                pointer = getattr(pointer, "weight")
            else:
                pointer = getattr(pointer, ln[0])
            if len(ln) >= 2:
                num = int(ln[1])
                pointer = pointer[num]
