...
class ResNet_Cifar(BasicModule):
    def __init__(self, block, layers, num_classes=10, thermometer=False, level=1,**kwargs):
...
def resnet20_cifar_defense(thermometer=False, level=1,**kwargs):
    model = ResNet_Cifar(BasicBlock, [3, 3, 3], thermometer=thermometer, level=level,**kwargs)
    return model
def getModel():
