...
class Evaluation_Base(object):
    ...
    def get_origin_sample(self, device, dataloader):
        ...
        for cln_xs, cln_ys in dataloader:
            origin_xs_numpy = self.adv_generate_batch_xs(
                device, cln_xs, origin_xs_numpy
            )
    ...
    def get_origin_ys(self, device, dataloader, dataloader_origin):
        ...
        for xs, ys in dataloader:
            targeted_ys_numpy = self.adv_generate_batch_ys(
                device, ys, targeted_ys_numpy
            )
    ...
    def setting_device(self, model_dir, model_name):
        ...
        if len(self.attack_method) == 1:
            ...
            att, att_name = (
                A_instance(model, device, self.IS_TARGETTED),
        ...
        # 参数不使用默认参数,而是用户自定义的,第一个是方法名称，第二个是次级名称，第三个是参数的xml文件
            ...
            if file_extension(self.attack_method[2]) in config_file_type_list:
                ...
                if self.attack_method[0] == "CORRUPT":
                    ...
                    att, att_name = (
                        A_instance(model, device, self.IS_TARGETTED, **args),
                        self.attack_method[0],
                    )  ############
    ...
    def setting_dataset(self, image_size, sample_path, label_path):
        # .npy 的格式
        if (
            file_extension(sample_path) == ".npy"
            and file_extension(self.label_path) == ".npy"
        ):
            # 批量化的图像攻击样本
        ...
        # 常见图像格式的
            ...
            mytransform = transforms.Compose(
                [
                    ...
                    transforms.CenterCrop(
                        (
                            min(self.crop_image_size[0], image_size[0]),
                            min(self.crop_image_size[1], image_size[1]),
                        )
                    ),
                    transforms.ToTensor(),
                ]
            ...
            if (
                len(self.data_type) == 2 and self.data_type[0] == "ImageNet"
            ) and self.data_type[1] == "withoutNormalize":
                mytransform = transforms.Compose(
                    [
                        transforms.Scale(image_size),
                        transforms.CenterCrop(
