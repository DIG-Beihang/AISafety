...
from utils.io_utils import (
    mkdir,
    get_label_lines,
    convertlist_to_numpy,
    gen_attack_adv_save_path,
    save_json,
    read_json,
    load_json,
    analyze_json,
...
def save_adv_result(
    ...
    if data_type == "cifar10" or data_type == "cifar100" or save_method == ".npy":
        ...
        np.save(
            save_as_black_path
            + "/{}_{}_advs.npy".format(attack_method, adv_xs.shape[0]),
            np.array(adv_xs),
        )
        path_adv_xs = save_as_black_path + "/{}_{}_advs.npy".format(
            attack_method, adv_xs.shape[0]
        )
        path_adv_xs_json = save_as_black_path + "/{}_{}_advs.json".format(
            attack_method, adv_xs.shape[0]
        )
        ...
        np.save(
            save_as_black_path
            + "/{}_{}_adv_preds_labels.npy".format(attack_method, adv_xs.shape[0]),
            np.array(ys_save),
        )
        path_adv_ys = save_as_black_path + "/{}_{}_adv_preds_labels.npy".format(
            attack_method, adv_xs.shape[0]
        )
        path_adv_ys_json = save_as_black_path + "/{}_{}_adv_preds_labels.json".format(
            attack_method, adv_xs.shape[0]
        )
        ...
        val_file_name_adv_preds = (
            save_val_path + "adv_preds_val_" + str(number) + ".txt"
        )
        ...
        with open(val_file_name_adv_preds, "w") as f_adv:
            with open(val_file_name_origins, "w") as f_origin:
                for i in range(adv_labels_numpy.shape[0]):
                    ...
                    adv_one_xs_resize = preprocess(
                        image=tensor_x, resize=resize_shape, device=device
                    )
                    vutils.save_image(
                        adv_one_xs_resize.cpu(), save_image_path + path_name
                    )
...
def save_numpy(inputs, path, discrube):
    np.save(
        path
        + "/{}_{}_outputs.npy".format("black_predict_" + discrube, inputs.shape[0]),
        np.array(inputs),
    )
...
class Rebust_Attack(Evaluation_Base):
    def __init__(
        ...
        self.model_defence_dir = (
            model_defence_dir  # '../Models/TestModel/resnet20_cifar.pt')
        )
        ...
        super(Rebust_Attack, self).__init__(
            attack_method,
            sample_path,
            label_path,
            image_origin_path,
    ...
    def gen_origin_Samples(self):
        ...
        cln_ys_numpy, targeted_ys_numpy = self.get_origin_ys(
            self.device, self.dataloader, self.dataloader_origin
        )
    ...
    def gen_origin_defense_Result(self):
        ...
        origin_xs_numpy = np.array(origin_xs)
        xs = torch.from_numpy(origin_xs_numpy)
        model_Defense = self.setting_model(
            self.model_defence_dir, self.defense_model_name, self.device
        )
    ...
    def estimate_Attack_compare(self, device, adv_samples_numpy):
        ...
        # 防御模型需要的结果
        model_Defense = self.setting_model(
            self.model_defence_dir, self.defense_model_name, device
        )
    ...
    def gen_Attack_Result(self, adv_samples_numpy=None):
        ...
        if not self.IS_COMPARE_MODEL:
            black_adv_outputs = self.estimate_Attack_uncompare(
                self.device, adv_samples_numpy
            )
        else:
            black_adv_outputs, black_defense_adv_outputs = self.estimate_Attack_compare(
                self.device, adv_samples_numpy
            )
    ...
    def gen_attack_Samples(self):
        ...
        dataloader_origin, dataset_origin = self.setting_dataset(
            self.Scale_ImageSize, self.image_origin_path, self.label_origin_path
        )
        self.model = model
        self.device = device
        ...
        if self.IS_WHITE:
            class_num_type, adv_samples = self.white_eval(
                att, model, device, dataloader
            )
        else:
...
class Rebust_Evaluate(object):
    def __init__(
        ...
        outputs_origin=None,
        outputs_adv=None,
        defense_outputs_origin=None,
        defense_outputs_adv=None,
        evaluation_method=None,
        IS_PYTHORCH_WHITE=None,
        IS_COMPARE_MODEL=None,
        IS_TARGETTED=None,
    ):
    ...
    def gen_evaluate(
        ...
        if self.IS_PYTHORCH_WHITE == True:
            ...
            if self.IS_COMPARE_MODEL:
                ...
                acac_eval_origin, eva_name_origin = (
                    E_instance(
                        outputs_origin,
                        outputs_adv,
                        defense_outputs_origin,
                        defense_outputs_adv,
                        device,
                        self.model,
                        self.model_Defence,
                ...
                rst = acac_eval_origin.evaluate(
                    adv_xs, cln_xs, cln_ys, adv_ys, target_pred, IS_TARGETTED
                )
                return rst
            else:
                E_instance = eval(self.evaluation_method)
                acac_eval, eva_name = (
                    E_instance(outputs_origin, outputs_adv, device, self.model),
                    self.evaluation_method,
                )
                rst = acac_eval.evaluate(
                    adv_xs, cln_xs, cln_ys, adv_ys, target_pred, IS_TARGETTED
                )
                return rst
        else:
            ...
            if self.IS_COMPARE_MODEL:
                ...
                print(str(self.evaluation_method))
                E_instance = eval(str(self.evaluation_method))
                acac_eval, eva_name = (
                    E_instance(outputs_origin, outputs_adv, device),
                    self.evaluation_method,
                )
                rst = acac_eval.evaluate(
                    adv_xs, cln_xs, cln_ys, adv_ys, target_pred, IS_TARGETTED
...
def save_cam_result(
    ...
    image, imgcv = get_image_from_path(
        image_origin_path, i, Crop_ImageSize, Crop_ImageSize
    )
    image_in = image
    imgcv_in = imgcv
    ...
    get_CAM_ImageList(
        image_in, imgcv_in, model, use_cuda, CAM_layer, CAM_path + model_name + "_", i
    )
    get_CAM_ImageList(
        adv_image,
        imgcv_in,
    ...
    if IS_COMPARE_MODEL:
        ...
        get_CAM_ImageList(
            ...
            use_cuda,
            CAM_layer,
            CAM_pathAttack + defense_model_name + "_",
            i,
        )


def Save_Eval_Visualization_Result(
    attName,
    data_type,
    file_name,
    Dict_path,
