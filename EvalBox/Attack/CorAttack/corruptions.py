...
def clipped_zoom(img, zoom_factor):
    ...
    top = (h - ch) // 2
    img = scizoom(
        img[top:top + ch, top:top + ch], (zoom_factor, zoom_factor, 1),
        order=1)
...
def defocus_blur(x, severity=1):
    ...
    channels = np.array(channels).transpose((1, 2,
                                             0))  # 3x224x224 -> 224x224x3
...
def frost(x, severity=1):
    c = [(1, 0.4), (0.8, 0.6), (0.7, 0.7), (0.65, 0.7), (0.6,
                                                         0.75)][severity - 1]
    ...
    x_start, y_start = np.random.randint(
        0, frost.shape[0] - H), np.random.randint(0, frost.shape[1] - W)
...
def snow(x, severity=1):
    ...
    x = c[6] * x + (1 - c[6]) * np.maximum(
        x,
        cv2.cvtColor(x, cv2.COLOR_RGB2GRAY).reshape(H, W, 1) * 1.5 + 0.5)
...
def jpeg_compression(x, severity=1):
    ...
    x=np.asarray(x)
    #x = PILImage.fromarray(x.astype('uint8')).convert('RGB')
...
def elastic_transform(image, severity=1):
    c = [
        (244 * 2, 244 * 0.7, 244 *
         0.1),  # 244 should have been 224, but ultimately nothing is incorrect
    ...
    return np.clip(
        map_coordinates(image, indices, order=1,
                        mode='reflect').reshape(shape), 0, 1) * 255
