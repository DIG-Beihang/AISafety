...
class EAD(Attack):
    def __init__(self, model=None, device=None,IsTargeted=None, **kwargs):
        ...
        @return: None
        '''
        super(EAD, self).__init__(model, device,IsTargeted)
    ...
    def generate(self, xs=None, ys=None):
        ...
        for search_for_c in range(self.binary_search_steps):
            ...
            slack = Variable(
                torch.from_numpy(copy_xs).to(device), requires_grad=True)
            ...
            for iteration_times in range(self.max_iter):
                ...
                kappa_t = torch.FloatTensor(
                    [self.kappa] * batch_size).to(device)
                target_loss_y = torch.max(
                    (output_y * targets_one_hot).sum(1)-
                     (output_y - 1e10 * targets_one_hot).max(1)[0], -1 * kappa_t)
                if targeted:
                    target_loss_y = torch.max(
                ...
                slack.data = new_image.data + (
                    (iteration_times /
                     (iteration_times + 3.0)) * (new_image - old_image)).data
