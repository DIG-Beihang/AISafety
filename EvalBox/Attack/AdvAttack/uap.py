...
from utils.get_train_validate_loader import get_mnist_train_validate_loader, get_cifar10_train_validate_loader, get_ImageNet_train_validate_loader
...
class UAP(Attack):
    ...
    def _parse_params(self, **kwargs):
        ...
        if self.dataset == "ImageNet":
            ...
            self.label_path = kwargs.get('label_path', '../Datasets/ImageNet/val_1000.txt')
        else:
            self.dir_name = kwargs.get('dir_name', '../Datasets/{}/'.format(self.dataset))
    ...
    def universal_pert(self, dataset, validation, device):
        ...
        print('starting to compute the universal adversarial perturbation with the training dataset ......')
        ...
        while ratio < self.fool_rate and iteration < self.uni_max_iter:
            ...
            for index, (image, label) in enumerate(dataset):
                original = torch.argmax(model(image.to(device)), 1) # prediction of the nature image
                ...
                if original == current:
                    ...
                    deepfool = DEEPFOOL(model=model, device=device,IsTargeted=self.IsTargeted, overshoot=self.deepfool_overshoot,
                                        max_iter=self.deepfool_max_iter)
                    _, delta, iter_num = deepfool._generate_one(x=perturbed_image,y=label,IsTargeted=self.IsTargeted)
                    # update the universal perturbation
                    if iter_num < self.deepfool_max_iter - 1:
                        uni_pert += torch.from_numpy(delta)
                        uni_pert = np.sign(uni_pert) * np.minimum(abs(uni_pert),self.epsilon)
            iteration += 1
            ...
            for index, (v_image, label) in enumerate(validation):
                ...
                original = torch.argmax(model(v_image.to(device)), 1)  # prediction of the nature image
    ...
    def generate(self, xs=None, ys=None):
        ...
        if not os.path.exists(uni_pert_npy):
            ...
            if self.dataset.upper() == "MNIST":
                dataset, validate = get_mnist_train_validate_loader(dir_name=self.dir_name, batch_size=1,
                                                                valid_size=0.9, shuffle=True)
            elif self.dataset== 'cifar10':
                dataset, validate = get_cifar10_train_validate_loader(dir_name=self.dir_name, batch_size=1,
                                                                    valid_size=0.9, shuffle=True)
            elif self.dataset == "ImageNet":
                dataset, validate = get_ImageNet_train_validate_loader(sample_path = self.sample_path,
                                                                       label_path = self.label_path,
                                                                       batch_size=1, valid_size=0.5, shuffle=True)
            ...
            x_n_new_resize= np.resize(x_n_new,(uni_pert_npy.shape[1],xs[0].shape[1],xs[0].shape[2]))
