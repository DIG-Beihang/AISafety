...
class ZOO(Attack):
    def \
            __init__(self, model=None, device=None, IsTargeted=None,**kwargs):
        '''
    ...
    def _parse_params(self, **kwargs):
        ...
        self.img_h = kwargs.get("img_h", 224)  #change for imagenet, setting to the image height
        self.img_w = kwargs.get("img_w", 224)   #change for imagenet, setting to the image width
        self.num_channels = kwargs.get("num_channel", 3)
        self.use_resize = kwargs.get("use_resize", False)  #setting True for imagenet ################
        ...
        self.reset_adam_after_found = kwargs.get("reset_adam", False)  #True for imagenet
        ...
        self.use_importance = kwargs.get("use_importance", False) #True for imagenet ################
    ...
    def init_setting_size(self,img_h,img_w,num_channels,use_resize,load_modifier):
        ...
        if load_modifier:
            self.real_modifier = np.load(load_modifier).reshape((1,) + small_single_shape)
        else:
    ...
    def resize_img(self, small_x, small_y, reset_only = False):
        ...
        if reset_only:
            ...
            self.real_modifier = zoom(self.real_modifier,(1,1,self.small_x/h_src,self.small_y/w_src))
    ...
    def _generate_one(self, x, y):
        ...
        var_img = Variable(
            torch.from_numpy(img).to(device), requires_grad=True)
        #print("var_img shape: ", var_img.shape)
        var_lab_one_hot = Variable(
            torch.FloatTensor(np.array(lab_one_hot)).to(device))
        ...
        for search_for_c in range(self.binary_search_steps):
            ...
            for iteration in range(self.max_iter):
                ...
                var = np.repeat(self.real_modifier, self.batch_size * 2 + 1, axis=0)  # (257,3,32,32)
                ...
                if self.use_resize:
                    modifier = zoom(modifier,(1, 1, self.img_h/modifier.shape[2], self.img_w/modifier.shape[3]))

                modifier = Variable(torch.from_numpy(modifier).to(device), requires_grad=True)
                if self.use_tanh:
                    perturbed_images = torch.tanh(
                        modifier + var_img) * half_range + mid_point  # (257,3,32,32) newimg[i] 原图像的某一个像素点+0.0001，newimg[i+1] 原图像的同一个像素点-0.0001
                else:
                ...
                if self.use_tanh:
                    l2dist = torch.sum((perturbed_images - (torch.tanh(var_img)* half_range + mid_point))**2, [1, 2, 3])
                ...
                other = torch.max((1-var_lab_one_hot) * prediction - (var_lab_one_hot*10000),1)[0]
                ...
                if targeted:
                    if self.use_log:
                        loss1 = torch.max(torch.stack((torch.log(other + 1e-30) - torch.log(real + 1e-30),
                                                       torch.zeros(2 * self.batch_size + 1, device=device)),1), 1)[0]
                    else:
                        loss1 = torch.max(torch.stack(
                            (other - real + self.confidence, torch.zeros(2 * self.batch_size + 1, device=device)),1), 1)[0]
                #untargeted attack
                else:
                    if self.use_log:
                        loss1 =  torch.max( torch.stack((torch.log(real+1e-30) - torch.log(other+1e-30),
                                                         torch.zeros(2*self.batch_size+1, device=device)),1), 1)[0]
                    else:
                        loss1 = torch.max( torch.stack((real - other + self.confidence,
                                                        torch.zeros(2*self.batch_size+1, device=device)),1), 1)[0]
                loss2 = l2dist
                ...
                if self.solver == "Adam":
                    for i in range(self.batch_size):
                        self.grad[i] = (losses[i * 2 + 1] - losses[i * 2 + 2]) / 0.0002  # (128,)
                    mt = self.mt[indice]
                    ...
                    corr = (np.sqrt(1 - np.power(self.beta2, epoch))) / (1 - np.power(self.beta1, epoch))
                    m = self.real_modifier.reshape(-1)
                    old_val = m[indice]
                    old_val -= self.lr * corr * mt / (np.sqrt(vt) + 1e-8)
                    if not self.use_tanh:
                        old_val = np.maximum(np.minimum(old_val, self.modifier_up[indice]), self.modifier_down[indice])
                    m[indice] = old_val
                    ...
                    for i in range(int(self.batch_size/2-1)):
                        ...
                        self.hess[i] = (losses[i * 2 + 1] - 2 * cur_loss + losses[i * 2 + 2]) / (0.0001 * 0.0001)
                    ...
                    # set it back to [-0.5, +0.5] region
                    if not self.use_tanh:
                        old_val = np.maximum(np.minimum(old_val, self.modifier_up[indice[hess_indice]]), self.modifier_down[indice[hess_indice]])
                    ...
                    old_val -= self.lr * corr * mt[adam_indice] / (np.sqrt(vt[adam_indice]) + 1e-8)
                    # old_val -= lr * grad[adam_indice]
                    # set it back to [-0.5, +0.5] region
                    if not self.use_tanh:
                        old_val = np.maximum(np.minimum(old_val, self.up[indice[adam_indice]]), self.down[indice[adam_indice]])
                    m[indice[adam_indice]] = old_val
                    self.adam_epoch[indice] = epoch + 1

                if self.save_modifier:
                    np.save('{}/iter{}'.format(self.save_modifier, iteration), self.real_modifier)
    ...
    def generate(self, xs=None, ys=None):
        ...
        self.init_setting_size(self.img_h, self.img_w, self.num_channels, self.use_resize, self.load_modifier)
        adv_xs = []
        #最后一层分类的类别数目获取
        parm = {}
