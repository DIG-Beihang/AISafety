...
class SPSA(Attack):
    ...
    def _project_perturbation(self,perturbation, norm, epsilon, input_image, clip_min=-np.inf,
                              clip_max=np.inf):
        """
    ...
    def spsa(self, x, y=None,norm=np.inf, clip_min=-np.inf, clip_max=np.inf,  early_stop_loss_threshold=None):
        '''
        @description:
        @param {
            x: [1xCxHxW]
        ...
        if len(x) != 1:
            ...
            for x_single, y_single in zip(x, y):
                adv_x_single = self.spsa( x=x_single.unsqueeze(0), y=y_single.unsqueeze(0),norm=np.inf, clip_min=-np.inf, clip_max=np.inf,  early_stop_loss_threshold=None)
        ...
        for i in range(nb_iter):
            def loss_fn(pert):
                ...
                return loss_multiplier * self._margin_logit_loss(logits, v_y.expand(len(pert)))

            spsa_grad = self._compute_spsa_gradient(loss_fn, v_x, delta=delta,
        ...
        if norm == np.inf:
            ...
            asserts.append(torch.all(torch.abs(
                torch.renorm(adv_x - copy_x, p=norm, dim=0, maxnorm=eps) - (adv_x - v_x)) < 1e-6))
