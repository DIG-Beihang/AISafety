...
class OM(Attack):
    ...
    def generate(self, xs=None, ys=None):
        ...
        C, H, W = xs.shape[1:]
        noise_raw = np.float32(np.random.normal(
            scale=self.noise_magnitude,
            size=(C * H * W, self.noise_count)).astype(np.float32))
        ...
        for search_for_c in range(self.binary_search_steps):
            ...
            for _ in range(self.max_iter):
                perturbed_img = torch.tanh(var_xs +
                ...
                for i in range(self.noise_count):
                    ...
                    c_loss = torch.max(
                        ...
                        torch.ones(batch_size,
                                   device=device) * self.kappa * -1)

                    if targeted:
                        c_loss = torch.max(
                            (prediction - 1e10 * labels_in_one_hot).max(1)[0]
                                -(prediction * labels_in_one_hot).sum(1),torch.ones(batch_size,
                                       device=device) * self.kappa * -1)
