...
class BA(Attack):
    ...
    def _init_sample(self, x, y, targeted,clip_min, clip_max):
        ...
        if targeted:
            ...
            for _ in range(self.init_size):
                ...
                tensor_random_img = Variable(torch.from_numpy(random_img).to(self.device))
            ...
            for _ in range(self.init_size):
                ...
                random_img_numpy = nprd.normal(loc=mean_, scale=2 * std_, size=x.shape).astype(x.dtype)
                random_img=np.expand_dims(random_img_numpy,axis=0)
                tensor_random_img = Variable(torch.from_numpy(random_img).to(self.device))
                output = self.model(tensor_random_img)
                random_class = torch.argmax(output, 1)
    ...
    def _orthogonal_perturb(self, delta, current_sample, original_sample):
        ...
        perturb = np.random.randn(original_sample.shape[0], original_sample.shape[1], original_sample.shape[2])
    ...
    def generate(self, xs=None, ys=None, target_flag=False):
        ...
        for i in range(rangenumbers):
            ...
            adversarial_sample=self._init_sample(origin_sample, preds[i], target_flag, 0, 1)
            for search_for_epsilon in range(self.binary_search_steps):
                ...
                for iteration_times in range(self.max_iter):
                     ...
                     for _ in range(self.sample_size):
                          ...
                          perturbed_image= adversarial_sample + self._orthogonal_perturb(delta_list[i],
                                                                                             adversarial_sample,
                                                                                              origin_sample)
                          perturbed_image=np.array(perturbed_image)
                          perturbed_image = np.clip(perturbed_image, self.lower_bound, self.upper_bound)
                          potential_perturbed_images.append(perturbed_image)
                     #potential_perturbed_images
                     var_images= Variable(torch.from_numpy(np.array(potential_perturbed_images)).to(self.device))
                     ...
                     if delta_ratio > 0:
                        ...
                        adversarial_sample = np.array(potential_perturbed_images)[np.where(satisfied)[0][0]]
                ...
                for _ in range(self.max_iter):
                     ...
                     potential_image = Variable(torch.from_numpy(potential_adv_expand).to(self.device))
