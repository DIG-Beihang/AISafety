...
class TKNC(Evaluation):
    def __init__(self, outputs_origin,outputs_adv, device, model, **kwargs):
        ...
        @return: None
        '''
        super(TKNC, self).__init__(outputs_origin,outputs_adv, device)
        self.model = model
    ...
    def evaluate(self,adv_xs=None, cln_xs=None, cln_ys=None,adv_ys=None,target_preds=None, target_flag=False):
        ...
        self.tknc_feature_map = [[] for i in range(len(self.module_name))]
        for key in range(len(self.module_name)):
            self.tknc_feature_map[key] = [0 for i in range(len(self.features_out_hook[key][0]))]
        ...
        for key in range(len(self.module_name)):
            # c表示第c个神经元
            for c in range(len(self.features_out_hook[key][0])):
                if self.tknc_feature_map[key][c] == 1:
                    covered += 1
                total += 1
