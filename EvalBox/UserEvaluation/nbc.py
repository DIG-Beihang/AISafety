...
class NBC(Evaluation):
    def __init__(self, outputs_origin,outputs_adv, device, model, **kwargs):
        ...
        @return: None
        '''
        super(NBC, self).__init__(outputs_origin,outputs_adv, device)
        self.model = model
    ...
    def evaluate(self,adv_xs=None, cln_xs=None, cln_ys=None,adv_ys=None,target_preds=None, target_flag=False):
        ...
        self.nbc_feature_map = [[] for i in range(len(self.module_name))]
        for key in range(len(self.module_name)):
            ...
            self.nbc_feature_map[key] = [0 for i in range(len(self.clean_feature_map[key][0]))]
        ...
        for key in range(len(self.module_name)):
            ...
            for k in range(len(self.features_out_hook[key])):
                ...
                for c in range(len(self.features_out_hook[key][k])):
                    if self.nbc_feature_map[key][c] == 1:
                        continue
                    # x表示key层的神经元c在第k个测试时的输出结果
                    # 对应上下界分别为 self.lower_feature_map[key][c]  self.upper_feature_map[key][c]
                    if self.features_out_hook[key][k][c].min() < self.lower_feature_map[key][c] or \
                      self.features_out_hook[key][k][c].max() > self.upper_feature_map[key][c]:
                        self.nbc_feature_map[key][c] = 1
        total = 0
        covered = 0
        for key in range(len(self.module_name)):
            # c表示第c个神经元
