        ...
        if not target_flag:
            for i in range(len(predicts)):
                if predicts[i] != labels[i]:
                    number += 1
                    dist_l0 += (
                        np.linalg.norm(np.reshape(pert[i], -1), ord=0) / NUM_PIXEL
                    )
                    norm_dist_l2 += np.linalg.norm(
                        np.reshape(cln_xs.numpy()[i] - adv_xs.numpy()[i], -1), ord=2
                    ) / (np.linalg.norm(np.reshape(cln_xs.numpy()[i], -1), ord=2))

                    dist_li += np.linalg.norm(
                        np.reshape(cln_xs.numpy()[i] - adv_xs.numpy()[i], -1),
                        ord=np.inf,
                    )
        else:
            for i in range(len(predicts)):
