...
class CCV(Evaluation_Defense):
    def __init__(self, outputs_origin,outputs_adv, defense_outputs_origin,defense_outputs_adv,device,  **kwargs):
        ...
        @return: None
        '''
        super(CCV, self).__init__(outputs_origin,outputs_adv, defense_outputs_origin,defense_outputs_adv,device)
    ...
    def evaluate(self,adv_xs=None, cln_xs=None, cln_ys=None,adv_ys=None,target_preds=None, target_flag=False):
        ...
        outputs_softmax,preds,_=self.get_Preds_Result(self.outputs_adv)
        outputs_softmax_defense, preds_defense ,_= self.get_Preds_Result(self.defense_outputs_adv)
            ...
            if not target_flag:
                ...
                if preds[i] != labels[i]:
                    ...
                    if preds_defense[i]!=labels[i]:
                            ...
                            prob_defense = outputs_softmax_defense[i, labels[i]]
                            dif_Prob+=abs(prob-prob_defense)
                            number+=1
            else:
                #分类正确在原始模型上的
                if preds[i] == labels[i]:
        ...
        if not number==0:
            ...
            ccv = dif_Prob / (number+MIN_COMPENSATION)
        return ccv
