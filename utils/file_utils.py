...
def random_choose_data(label_path,random_ratio,division):
   ...
   # print("item_numbers",item_numbers)
    ...
    slice_initial = random.sample(lines, selected_number)  # if don't change this ,it will be all the same
...
def show_attack_result(origin_sample,adv_sample,origin_outputs,adv_outputs,dictionary):
    return
...
def xmlparser(file_path):
    ...
    for param in  params:
        if param.hasAttribute("title"):
           ...
           # print(contentname)
            dict[contentname]=param.getElementsByTagName(contentname)[0].childNodes[0].data
            #print(dict)
    return(dict)
