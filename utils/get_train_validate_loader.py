...
def get_ImageNet_train_validate_loader(sample_path, label_path, batch_size, valid_size=0.1, shuffle=True, random_seed=100, num_workers=1):
    """
    ...
    dataset = EvalDataset(root_path=sample_path, label_path=label_path, origin_path=None, origin_label_path=None,
                          data_type="ImageNet",
                          image_size=(224,224), transform=transform,
                          ratio=1.0)
...
def get_cifar10_train_validate_loader(dir_name, batch_size, valid_size=0.1, augment=True, shuffle=True, random_seed=100, num_workers=1):
    ...
    valid_cifar10_dataset = torchvision.datasets.CIFAR10(root=dir_name, train=False, download=True, transform=valid_transform)
...
def get_mnist_train_validate_loader(dir_name, batch_size, valid_size=0.1, shuffle=True, random_seed=100, num_workers=1):
    ...
    train_mnist_dataset = torchvision.datasets.MNIST(root=dir_name, train=True, transform=transforms.ToTensor(), download=False)
    valid_mnist_dataset = torchvision.datasets.MNIST(root=dir_name, train=True, transform=transforms.ToTensor(), download=False)
