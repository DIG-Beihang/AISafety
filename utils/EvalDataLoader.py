...
class EvalDataset(Dataset):
    # 初始化
    def __init__(self, root_path,label_path,origin_path=None,origin_label_path=None,data_type=None,image_size=None,transform=None,ratio=None):
        # 读入数据
        ...
        if not origin_path == None and not origin_label_path == None:
            imgs_origin, labels_origin = load_data(data_type, origin_path, origin_label_path, ratio)
    ...
    def __getitem__(self, idx):
        ...
        if not self.imgs_origin==None and not self.labels_origin==None:
            img_origin = self.loader(os.path.join(self.origin_path, self.imgs_origin[idx]))
