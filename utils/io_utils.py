...
def get_datasets_without_mask(imgs_dir,label_dir,height, width, channels, train_test="null"):
...
def get_datasets(imgs_dir,label_dir,bordermask_dir, height, width, channels, train_test="null"):
    Nimgs = len(os.listdir(imgs_dir))
    imgs = np.empty((Nimgs,height,width,channels))
    print(imgs.shape)
    groundTruth = np.empty((Nimgs,height,width))
    ...
    for path, subdirs, files in os.walk(imgs_dir): #list all files, directories in the path
        for i in range(len(files)):
            ...
            if (origin_img.shape[0] == height) and (origin_img.shape[1] == width):
                ...
                print("padding image.......")
                origin_img = np.asarray(img)
                target_h, target_w, img_h, img_w = height, width, origin_img.shape[0], origin_img.shape[1]
                ...
                padded_img[(target_h - img_h) // 2:(target_h - img_h) // 2 + img_h,
                (target_w - img_w) // 2:(target_w - img_w) // 2 + img_w, ...] = origin_img
            ...
            if (origin_manual.shape[0] == height) and (origin_manual.shape[1] == width):
                ...
                target_h, target_w, img_h, img_w = height, width, origin_manual.shape[0], origin_manual.shape[1]
                ...
                padded_manual[(target_h - img_h) // 2:(target_h - img_h) // 2 + img_h,
                (target_w - img_w) // 2:(target_w - img_w) // 2 + img_w, ...] = origin_manual
...
def get_cam_list(self,cam_path,sample_index_list):
        ...
        for study in test_study_list:
            cam_list.append([os.path.join(abspath, study, 'cam_' +index+ '.jpg') for index in sample_index_list] )
            orig_list.append([os.path.join(abspath, study, 'orig_' +index+ '.jpg') for index in sample_index_list])
...
def get_top_k_list(self, top_k_path):
    ...
    test_study_list = os.listdir(abspath)
    for study in test_study_list:
        imgs_list.append([os.path.join(abspath, study, ch + '.jpg') for ch in channels_list])
...
def result_as_html(base_abspath, x_list, predict_y_list, y_list=None):
    ...
    html_content = '<html><head><title>priction_title</title>' \
    ...
    for index in range(len(x_list)):
        ...
        html_content = html_content + '<hr><h3 class="hl">'+imgs[0][0:imgs[0].rfind('/')]+'</h3>'
        #imgs = [imgs]
        for i in range(len(imgs)):
            html_content =  html_content + '<div class="card"><h3>' + imgs[
                i].split('/')[-1] + '</h3><img src="'+os.path.relpath(imgs[i],base_abspath)+'" width='+str(html_img_width)+' height='+str(html_img_width)+' /></div>'

        if y_list:
            groundTruth = y_list[index]
            for i in range(len(groundTruth)):
                html_content = html_content + '<div class="card"><h3>' + groundTruth[
                    i].split('/')[-1] + '</h3><img src="' + os.path.relpath(groundTruth[i],base_abspath) + '" width=' + str(html_img_width) + ' height=' + str(html_img_width) + ' /></div>'

        for i in range(len(results)):
            html_content = html_content + '<div class="card"><h3>' + results[
                i].split('/')[-1] + '</h3><img src="'+os.path.relpath(results[i],base_abspath)+'" width=' + str(html_img_width) + ' height=' + str(html_img_width) + ' /></div>'
...
def get_resized_img_from_dir(MSI_filename, MSI_image_name, original_row, original_col, resizeratio):
...
def SaveWithJson(DIR, save_type, table_name = '', model_name = '', evaluation_name = '', row = '', value = 0):
    # 检测结果文件是否存在
...
def SaveWithJson_Result(DIR, save_type, attName = '', Attack_file_name = '', evaluation_name = '', value = 0):
    # 检测结果文件是否存在
    if not os.path.exists(DIR):
        os.makedirs(DIR)
    if not os.path.exists(DIR + "/result.txt"):
...
def center_Crop(Image,ImageScale_size,crop_size):
    ...
    cropImg = imgcv[center_x-crop_size[0]//2:center_x+crop_size[0]//2,center_y-crop_size[1]//2:center_y+crop_size[1]//2]
    return cropImg
