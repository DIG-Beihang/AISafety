...
from .io_utils import load_hdf5, write_hdf5
from .img_utils import visualize, masks_Unet2
from .img_utils import group_images

from .img_utils import my_PreProc2
...
def get_data_predict_overlap(
    ...
    test_imgs_adjust = my_PreProc2(test_imgs_original)

    # extend both images and masks so they can be divided exactly by the patches dimensions
    # test_imgs = test_imgs[0:Imgs_to_test,:,:,:]
    # test_masks = test_masks[0:Imgs_to_test,:,:,:]
    test_imgs = paint_border_overlap(
        test_imgs_adjust, patch_height, patch_width, stride_height, stride_width
    )
    ...
    print(
        "test PATCHES images range (min-max): "
...
def get_data_training(
    ...
    train_masks = np.reshape(
        train_masks,
        (train_masks.shape[0], train_masks.shape[1], train_masks.shape[2], 1),
    )
    # visualize(group_images(train_imgs_original[0:20,:,:,:],5),'imgs_train')#.show()  #check original imgs train
    print("train_imgs_original:" + str(train_imgs_original.shape))
    print("train_masks:" + str(train_masks.shape))
    t2 = datetime.now()
    print("load DATA cost:" + str(t2 - t1))
    train_imgs = my_PreProc2(train_imgs_original)
    print("train_imgs:" + str(train_imgs.shape))
    train_masks = train_masks / 255.0

    # train_imgs = train_imgs[:,9:574]  #cut bottom and top so now it is 565*565
    # train_masks = train_masks[:,9:574]  #cut bottom and top so now it is 565*565
    train_imgs = train_imgs[:, :]  # modified by cq
    train_masks = train_masks[:, :]  # modified by cq
    print("train_imgs:" + str(train_imgs.shape))
    print("train_masks:" + str(train_masks.shape))
    data_consistency_check(train_imgs, train_masks)
    t3 = datetime.now()
    print("cut b and t and check out" + str(t3 - t2))
    # check masks are within 0-1
    ...
    print(
        "train images range (min-max): "
        + str(np.min(train_imgs))
        + " - "
    ...
    t4 = datetime.now()
    print("extract_random:" + str(t4 - t3))
    print("\ntrain PATCHES images/masks shape:")
    print(patches_imgs_train.shape)
    print(
        "train PATCHES images range (min-max): "
        + str(np.min(patches_imgs_train))
        + " - "
        + str(np.max(patches_imgs_train))
    )
    print("patches_imgs_train:" + str(patches_imgs_train.shape))
    print("patches_masks_train:" + str(patches_masks_train.shape))
    patches_masks_train = masks_Unet2(patches_masks_train)
    print("after reshape, patches_masks_train:" + str(patches_masks_train.shape))
    t5 = datetime.now()

    print("masks unet:" + str(t5 - t4))
    return (
        patches_imgs_train,
        patches_masks_train,
...
def get_data_testing(
    ...
    test_imgs = my_PreProc2(test_imgs_original)
...
def get_data_testing_overlap(
    ...
    test_imgs = test_imgs[0:Imgs_to_test, :, :, :]
    # test_masks = test_masks[0:Imgs_to_test,:,:,:]
    test_masks = test_masks[0:Imgs_to_test, :, :]  # modified by cq
    test_imgs = paint_border_overlap(
        test_imgs, patch_height, patch_width, stride_height, stride_width
    )
...
def extract_random(full_imgs, full_masks, patch_h, patch_w, N_patches, inside=True):
    ...
    assert (
        full_imgs.shape[3] == 1 or full_imgs.shape[3] == 3
    )  # check the channel is 1 or 3
    assert full_masks.shape[3] == 1  # masks only black and white
    assert (
        full_imgs.shape[1] == full_masks.shape[1]
        and full_imgs.shape[2] == full_masks.shape[2]
    )
    ...
    for i in range(full_imgs.shape[0]):  # loop over the full images
        ...
        while k < patch_per_img:
            ...
            if inside == True:
                if (
                    is_patch_inside_FOV(x_center, y_center, img_w, img_h, patch_h)
                    == False
                ):
                    continue
            patch = full_imgs[
...
def paint_border_overlap(full_imgs, patch_h, patch_w, stride_h, stride_w):
    ...
    if leftover_h != 0:  # change dimension of img_h
        ...
        tmp_full_imgs[
            0 : full_imgs.shape[0], 0:img_h, 0:img_w, 0 : full_imgs.shape[3]
        ] = full_imgs
...
def pred_only_FOV(data_imgs, data_masks, original_imgs_border_masks):
    ...
    assert (
        data_imgs.shape[3] == 1 and data_masks.shape[3] == 1
...
def inside_FOV_DRIVE(i, x, y, DRIVE_masks):
    ...
    if (
        x >= DRIVE_masks.shape[2] or y >= DRIVE_masks.shape[1]
