...
def plotresult(bg_img_path, manual_img_path, predict_img_path, dst_path):
    ...
    _,contours2, hierarchy2 = cv2.findContours(mask_thresh1_predcit, cv2.RETR_TREE, cv2.CHAIN_APPROX_SIMPLE)
    ...
    for c in contours1:
        ...
        if (cX == cY == 0):
            cX = c[0][0][0]
            cY = c[0][0][1]
        cv2.circle(img, (cX, cY), 12, (255, 0, 0), 2) #-1 means fullfill circle, candidate blue
