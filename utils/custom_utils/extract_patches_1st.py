...
from utils.io_utils import load_hdf5
from utils.img_utils import visualize, group_images, my_PreProc2
import sys
...
def get_data_predict_overlap(
    ...
    ### test
    print("get_data_predict_overlap")
    img = imgPredict
    test_imgs_original = []
...
def get_data_training(
    ...
    for i in range(num_lesion):
        train_masks_temp = load_hdf5(
            DRIVE_train_groudTruth + str(i + 1) + ".hdf5"
        )  # [img_id:img_id+1]#masks always the same
    ...
    print(
        "train images 0 range (min-max): "
        + str(np.min(train_imgs[:, 0]))
        + " - "
        + str(np.max(train_imgs[:, 0]))
    )
    print(
        "train images 1 range (min-max): "
        + str(np.min(train_imgs[:, 1]))
        + " - "
        + str(np.max(train_imgs[:, 1]))
    )
    print(
        "train images 2 range (min-max): "
        + str(np.min(train_imgs[:, 2]))
        + " - "
        + str(np.max(train_imgs[:, 2]))
    )
...
def get_data_testing(
    ...
    test_masks = np.zeros(
        [
            total_data,
            num_lesion,
            test_imgs_original.shape[2],
            test_imgs_original.shape[3],
        ]
    )

    for i in range(num_lesion):
        test_masks_temp = load_hdf5(
            DRIVE_test_groudTruth + str(i + 1) + ".hdf5"
        )  # [img_id:img_id+1]#masks always the same
    ...
    print(test_imgs.shape)
    print(
        "test images range (min-max): "
        + str(np.min(test_imgs))
        + " - "
...
def get_data_testing_overlap(
    ...
    test_masks = paint_border_overlap(
        test_masks, patch_height, patch_width, stride_height, stride_width
    )
    # check masks are within 0-1
    assert np.max(test_masks) == 1 and np.min(test_masks) == 0
...
def extract_random(full_imgs, full_masks, patch_h, patch_w, N_patches, inside=True):
    ...
    assert (
        full_imgs.shape[2] == full_masks.shape[2]
...
def recompone_overlap(
    preds, img_h, img_w, patch_height, patch_width, stride_h, stride_w
):
    ...
    print(
        "According to the dimension inserted, there are "
        + str(N_full_imgs)
        + " full images (of "
        + str(img_h)
        + "x"
    ...
    full_prob = np.zeros(
        (N_full_imgs, preds.shape[1], img_h, img_w)
                ...
                for w in range((img_w - patch_w) // stride_w + 1):
                    for y in range(patch_h):
                        for x in range(patch_w):
                            for c in range(3):
                                if (
                                    full_prob[i, c, h * stride_h + y, w * stride_w + x]
                                    < preds[k, c, y, x]
                                ):
                                    full_prob[
                                        i, c, h * stride_h + y, w * stride_w + x
                                    ] = preds[k, c, y, x]
                    k = k + 1
        final_avg = full_prob
        print("using max")
...
def pred_only_FOV(data_imgs, data_masks, original_imgs_border_masks):
    ...
    assert (
        data_imgs.shape[1] == 1 and data_masks.shape[1] == 1
