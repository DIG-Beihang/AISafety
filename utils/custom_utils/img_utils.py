...
def pred_to_imgs2(pred,patch_height, patch_width,num_lesion_class,mode="original"):
    ...
    if mode=="original":
        ...
        print("mode " +str(mode) +" not recognized, it can be 'original' or 'threshold'")
        exit()
    pred_images = np.reshape(pred_images,(pred_images.shape[0],patch_height,patch_width,2))
    return pred_images
...
def get_center_points_from_mask(mask):
    ...
    _, contours1, hierarchy1 = cv2.findContours(mask_thresh1, cv2.RETR_TREE, cv2.CHAIN_APPROX_SIMPLE)
...
def generateCircleMask(img,sizedilation=4,threshmin=10):
    ...
    elem1 = cv2.getStructuringElement(cv2.MORPH_ELLIPSE, (int(sizedilation/2), int(sizedilation/2)))
    ...
    _, contours, hierarchy = cv2.findContours(erodeimage, cv2.RETR_TREE, cv2.CHAIN_APPROX_SIMPLE)
