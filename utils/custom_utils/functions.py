...
def MultiChannelData(MSI_filename, MSI_image_name, original_row, original_col, resizeratio):
...
def GenTrainImages_positive(MSI_image,Mask_image, MSI_image_save_file, PosOrNeg, case_num, patch_size, scale_ratio = -1):
    ...
    for i in range(0, len(contours)):
        ...
        ############################ crop centered image
        crop_mask = Mask_image_padding[(center[1]-int(patch_size/2)):(center[1]+int(patch_size/2)), (center[0]-int(patch_size/2)):(center[0]+int(patch_size/2)),0]
        ...
        if aug_rotation == 'yes':
            ...
            for roi in range(aug_rotation_num):
                ...
                crop_mask_name = save_dir_mask + '/' + str(case_num) + '_CR_' + str(i) + '_' + str(roi) + '_' +str(crop_num) + '.jpg'
                ...
                crop_MSI_name = save_dir_image + '/' + str(case_num) + '_CR_' + str(i) + '_' + str(roi) + '_' + str(crop_num) + '.jpg'
        ...
        if scale_ratio > -1:
            crop_mask = Mask_image_padding[(center[1] - int(patch_size*scale_ratio/ 2)):(center[1] + int(patch_size*scale_ratio/2)),
                        (center[0] - int(patch_size*scale_ratio / 2)):(center[0] + int(patch_size*scale_ratio / 2)),0]
            ...
            if aug_rotation == 'yes':
                ...
                for roi in range(aug_rotation_num):
                    ...
                    crop_mask_R = crop_mask_R[:, :, 0]
                    crop_mask_name = save_dir_mask2 + '/' + str(case_num) + '_CR_' + str(i) + '_' + str(roi) + '_'+ str(crop_num) + '.jpg'
                    cv2.imwrite(crop_mask_name, crop_mask_R)
                    ...
                    crop_MSI_image_R = subimage(MSI_image_padding, center, rotate_theta, int(patch_size * scale_ratio),
                                                int(patch_size * scale_ratio))
                    crop_MSI_name = save_dir_image2 + '/' + str(case_num) + '_CR_' + str(i) + '_' + str(roi) + '_'+ str(crop_num) + '.jpg'
                    cv2.imwrite(crop_MSI_name, crop_MSI_image_R)
        ...
        if aug_translation == 'yes':
            ...
            for tri in range(aug_translation_num):
                ...
                crop_mask = Mask_image_padding[(center_trans[1] - int(patch_size / 2)):(center_trans[1] + int(patch_size / 2)),
                            (center_trans[0] - int(patch_size / 2)):(center_trans[0] + int(patch_size / 2)),0]
                ...
                if aug_rotation == 'yes':
                    ...
                    for roi in range(aug_rotation_num):
                        ...
                        crop_mask_R = crop_mask_R[:, :, 0]
                        crop_mask_name = save_dir_mask + '/' + str(case_num) + '_CTR_' + str(i) + '_' + str(roi) + '_'+str(
                            crop_num) + '_' + str(tri) + '.jpg'
                        cv2.imwrite(crop_mask_name, crop_mask_R)
                        mask_list.append(crop_mask_R)

                        crop_MSI_image_R = subimage(MSI_image_padding, center_trans, rotate_theta, patch_size, patch_size)
                        crop_MSI_name = save_dir_image + '/' + str(case_num) + '_CTR_' + str(i) + '_' + str(roi) + '_'+str(
                            crop_num) + '_' + str(tri) + '.jpg'
                        cv2.imwrite(crop_MSI_name, crop_MSI_image_R)
                        img_list.append(crop_MSI_image_R)
                    ...
                    crop_mask = Mask_image_padding[(center_trans[1] - int(patch_size * scale_ratio / 2)):(
                        center_trans[1] + int(patch_size * scale_ratio / 2)),
                                (center_trans[0] - int(patch_size * scale_ratio / 2)):(
                                    center_trans[0] + int(patch_size * scale_ratio / 2)),0]
                    ...
                    if aug_rotation == 'yes':
                        ...
                        for roi in range(aug_rotation_num):
                            ...
                            crop_mask_R = subimage(Mask_image_padding, center_trans, rotate_theta, int(patch_size * scale_ratio),
                                                   int(patch_size * scale_ratio))
                            crop_mask_R = crop_mask_R[:, :, 0]
                            crop_mask_name = save_dir_mask2 + '/' + str(case_num) + '_CTR_' + str(i) + '_' +str(roi) + '_'+ str(
                                crop_num) + '_' + str(tri) + '.jpg'
                            cv2.imwrite(crop_mask_name, crop_mask_R)
                            sub_mask_list.append(crop_mask_R)
                            ...
                            crop_MSI_image_R = subimage(MSI_image_padding, center_trans, rotate_theta,
                                                        int(patch_size * scale_ratio),
                                                        int(patch_size * scale_ratio))
                            crop_MSI_name = save_dir_image2 + '/' + str(case_num) + '_CTR_' + str(i) + '_' +str(roi) + '_'+ str(
                                crop_num) + '_' + str(tri) + '.jpg'
                            cv2.imwrite(crop_MSI_name, crop_MSI_image_R)
                            sub_img_list.append(crop_MSI_image_R)
                print('Iteration:')
        ...
        for col in range(0,width-patch_size,step):
            ...
            if flag == 0:
                crop_mask = Mask_image_padding[(y + size_1):(y + h + size_1),
                            (x + size_1):(x + w + size_1),0]  # 先用y确定高，再用x确定宽
                ...
                if scale_ratio > -1:
                    crop_mask = Mask_image_padding[(y - size_2 + size_1):(y - size_2 + H + size_1),
                                (x - size_2 + size_1):(x - size_2 + W + size_1),0]  # 先用y确定高，再用x确定宽
...
def GenTrainImages_augmentation(MSI_image, Mask_image, MSI_image_save_file, PosOrNeg, case_num,patch_size,scale_ratio, padding_num):
    img_list = []
    mask_list = []
    sub_img_list = []
    sub_mask_list = []
    ...
    BLACK = [0, 0, 0]
    Mask_image_padding = cv2.copyMakeBorder(Mask_image, int(height / 2), int(height / 2), int(height / 2), int(height / 2),
                                            cv2.BORDER_CONSTANT, value=BLACK)
    MSI_image_padding = cv2.copyMakeBorder(MSI_image, int(height / 2), int(height / 2), int(height / 2), int(height / 2),
                                           cv2.BORDER_CONSTANT, value=BLACK)
    ...
    ############################ save original image
    if padding_num > 0:
        crop_mask = Mask_image_padding[(int(Height / 2) - int(height / 2)-padding_num):(int(Height / 2) + int(height / 2)+padding_num),
                    (int(Width / 2) - int(width / 2) - padding_num):(int(Width / 2) + int(width / 2) + padding_num),0]
        crop_MSI_image = MSI_image_padding[(int(Height / 2) - int(height / 2) - padding_num):(int(Height / 2) + int(height / 2) + padding_num),
                    (int(Width / 2) - int(width / 2) - padding_num):(int(Width / 2) + int(width / 2) + padding_num)]

    else:
        crop_mask = Mask_image_padding[(int(Height / 2) - int(height / 2)):(int(Height / 2) + int(height / 2)),
                    (int(Width / 2) - int(width / 2)):(int(Width / 2) + int(width / 2)),0]
        crop_MSI_image = MSI_image_padding[(int(Height / 2) - int(height / 2)):(int(Height / 2) + int(height / 2)),
    ...
    if aug_rotation == 'yes':
        ...
        for roi in range(aug_rotation_num):
            rotate_theta = float(rotate_theta_random[roi])
            crop_mask_R = subimage(Mask_image_padding[:,:,0], center , rotate_theta, size, size)
            crop_mask_name = save_dir_mask + '/' + str(case_num) + '_ori_'+ str(roi+1) + '.jpg'
            cv2.imwrite(crop_mask_name, crop_mask_R)
            mask_list.append(crop_mask_R)

            crop_MSI_image_R = subimage(MSI_image_padding, center, rotate_theta, size, size)
            crop_MSI_name = save_dir_image + '/' + str(case_num) + '_ori_'+ str(roi+1) + '.jpg'
...
def GenTrainImages_positive_center(MSI_image, Mask_image, MSI_image_save_file, PosOrNeg, case_num, patch_size, step = 10, scale_ratio=-1):
    img_list = []
    mask_list = []
    sub_img_list = []
    sub_mask_list = []
    ...
    for row in range(0, height - patch_size, step):
        for col in range(0, width - patch_size, step):
            ...
            if crop_mask[center_row-1, center_col-1, 0] >= 200 or crop_mask[center_row, center_col-1, 0] >= 200 or crop_mask[center_row-1, center_col, 0] >= 200 or crop_mask[center_row, center_col, 0]>= 200:
                flag = 1
            # for sub_row in range(0, patch_size):
            #     for sub_col in range(0, patch_size):
            #         if crop_mask[sub_row, sub_col, 0] >= 200:
            ...
            if flag == 1:
                crop_mask = Mask_image_padding[(y + size_1):(y + h + size_1),
                            (x + size_1):(x + w + size_1), 0]  # 先用y确定高，再用x确定宽
                crop_mask_name = save_dir_mask + '/' + str(case_num) + '_Cen_' + str(row) + '_' + str(col) + '.jpg'
                cv2.imwrite(crop_mask_name, crop_mask)
                mask_list.append(crop_mask)
                print('save1:')
                ...
                crop_MSI_image = MSI_image_padding[(y + size_1):(y + h + size_1),
                                 (x + size_1):(x + w + size_1)]  # 先用y确定高，再用x确定宽
                crop_MSI_image_name = save_dir_image + '/' + str(case_num) + '_Cen_' + str(row) + '_' + str(
                    col) + '.jpg'
                print('save2:')
                cv2.imwrite(crop_MSI_image_name, crop_MSI_image)
                img_list.append(crop_MSI_image)
                ...
                if aug_rotation == 'yes':
                    ...
                    for roi in range(aug_rotation_num):
                        ...
                        crop_mask_R = subimage(Mask_image_padding, center, rotate_theta, patch_size, patch_size)
                        crop_mask_R = crop_mask_R[:, :, 0]
                        crop_mask_name = save_dir_mask + '/' + str(case_num) + '_CenR_' + str(row) + '_' + str(col) + '.jpg'
                        cv2.imwrite(crop_mask_name, crop_mask_R)
                        mask_list.append(crop_mask_R)
                        crop_MSI_image_R = subimage(MSI_image_padding, center, rotate_theta, patch_size, patch_size)
                        crop_MSI_name = save_dir_image +  '/' + str(case_num) + '_CenR_' + str(row) + '_' + str(col) + '.jpg'
                        cv2.imwrite(crop_MSI_name, crop_MSI_image_R)
                        img_list.append(crop_MSI_image_R)
                ...
                if scale_ratio > -1:
                    crop_mask = Mask_image_padding[(y - size_2 + size_1):(y - size_2 + H + size_1),
                                (x - size_2 + size_1):(x - size_2 + W + size_1),0]  # 先用y确定高，再用x确定宽
                    crop_mask_name = save_dir_mask2 + '/' + str(case_num) + '_Cen_' + str(row) + '_' + str(col) + '.jpg'
                    print('save3:')
                    cv2.imwrite(crop_mask_name, crop_mask)
                    sub_mask_list.append(crop_mask)
                    ...
                    crop_MSI_image = MSI_image_padding[(y - size_2 + size_1):(y - size_2 + H + size_1),
                                     (x - size_2 + size_1):(x - size_2 + W + size_1)]  # 先用y确定高，再用x确定宽
                    crop_MSI_image_name = save_dir_image2 + '/' + str(case_num) + '_Cen_' + str(row) + '_' + str(
                        col) + '.jpg'
                    print('save4:')
                    cv2.imwrite(crop_MSI_image_name, crop_MSI_image)
                    sub_img_list.append(crop_MSI_image)
                    ...
                    if aug_rotation == 'yes':
                        # rotate_theta_random = random.sample(range(rotation_theta, 360, rotation_theta),
                        #                                     aug_rotation_num)
                        center = [int(x + size_1 + W / 2- size_2), int(y + size_1 + H / 2- size_2)]
                        for roi in range(aug_rotation_num):
                            rotate_theta = float(rotate_theta_random[roi])
                            crop_mask_R = subimage(Mask_image_padding, center, rotate_theta,
                                                   int(patch_size * scale_ratio),
                                                   int(patch_size * scale_ratio))
                            crop_mask_R = crop_mask_R[:, :, 0]
                            crop_mask_name = save_dir_mask2 + '/' + str(case_num) + '_CenR_' + str(row) + '_' + str(col) + '.jpg'
                            cv2.imwrite(crop_mask_name, crop_mask_R)
                            sub_mask_list.append(crop_mask_R)

                            crop_MSI_image_R = subimage(MSI_image_padding, center, rotate_theta,
                                                        int(patch_size * scale_ratio),
                                                        int(patch_size * scale_ratio))
                            crop_MSI_name = save_dir_image2 + '/' + str(case_num) + '_CenR_' + str(row) + '_' + str(col) + '.jpg'
                            cv2.imwrite(crop_MSI_name, crop_MSI_image_R)
                            sub_img_list.append(crop_MSI_image_R)
    return img_list, mask_list, sub_img_list, sub_mask_list

def GenTrainImages_negative_notcenter(MSI_image, Mask_image, MSI_image_save_file, PosOrNeg, case_num, patch_size, step = 10, scale_ratio=-1):
    ...
    for row in range(0,height-patch_size,step):
        for col in range(0,width-patch_size,step):
            ...
            if crop_mask[sub_row, sub_col, 0] < 100 and crop_mask[sub_row-1, sub_col, 0] < 100 and crop_mask[sub_row, sub_col-1, 0] < 100 and crop_mask[sub_row-1, sub_col-1, 0] < 100:
                flag = 0
            # for sub_row in range(0,patch_size):
            #     for sub_col in range(0,patch_size):
            #         if crop_mask[sub_row,sub_col,0] >=200:
...
def GenTrainImages_testpatch(MSI_image, Mask_image, MSI_image_save_file, PosOrNeg, case_num, patch_size,
                            step=10, scale_ratio=-1):
    img_list = []
    mask_list = []
    sub_img_list = []
    sub_mask_list = []
    ...
    step = patch_size
    save_dir_image, save_dir_image2 = SaveImageName(MSI_image_save_file, PosOrNeg, patch_size,
                                                    scale_ratio)
    save_dir_mask, save_dir_mask2 = SaveMaskName(MSI_image_save_file, PosOrNeg, patch_size, scale_ratio)
    ...
    for row in range(0, height, step):
        for col in range(0, width, step):
            ...
            crop_mask = Mask_image_padding[(y + size_1):(y + h + size_1),
                        (x + size_1):(x + w + size_1), 0]  # 先用y确定高，再用x确定宽
            crop_mask_name = save_dir_mask + '/' + str_casenum +  '_' +str(patch_num) + '.jpg'
            cv2.imwrite(crop_mask_name, crop_mask)
            mask_list.append(crop_mask)
            print('save1:')
            ...
            crop_MSI_image = MSI_image_padding[(y + size_1):(y + h + size_1),
                             (x + size_1):(x + w + size_1)]  # 先用y确定高，再用x确定宽
            crop_MSI_image_name = save_dir_image + '/' + str_casenum +  '_' +str(patch_num) + '.jpg'
            print('save2:')
            cv2.imwrite(crop_MSI_image_name, crop_MSI_image)
            img_list.append(crop_MSI_image)
...
def CreateSaveFile(MSI_image_save_file,PosOrNeg,patch_size,scale_ratio):
    ...
    if scale_ratio > 0:
        ...
        save_dir2 = os.path.join(MSI_image_save_file, patch_name0,patch_name2, 'images')
        if os.path.exists(save_dir2) == False:
            os.makedirs(save_dir2)
